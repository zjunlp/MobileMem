...
selection and batched ``*_info`` generation. Logs to the shared ``document``
...
No extra text, ONLY the JSON array."""
    ...
    for attempt in range(STAGE_RETRY_TIMES):
        try:
            ...
            logger.warning(f"[LLM-select] Attempt {attempt+1}/{STAGE_RETRY_TIMES} failed: {e}")
...
def select_friend_events(events):
    """Select events that already have friend_info (generated by annual_events or other steps)."""
        ...
        try:
            ...
            if match:
                try:
                    ...
                    # Keep only structurally valid items: a dict carrying event_id and
                    # a non-empty *_info dict. The caller only skips info that is None,
                    # so an empty {} would be written into the event record as if it
                    ...
                    if valid:
                        return valid
                    logger.warning(f"[LLM] Parsed array has no valid {app_type}_info items (batch of {len(events)})")
            ...
            logger.warning(f"[LLM] Attempt {attempt+1}/{STAGE_RETRY_TIMES} failed: {e}")
            if attempt < STAGE_RETRY_TIMES - 1:
                time.sleep(3 * (2 ** attempt))
    return []
